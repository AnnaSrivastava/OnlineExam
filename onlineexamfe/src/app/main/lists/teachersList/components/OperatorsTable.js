...
function OperatorsTable(props) {
	...
	useEffect(() => {
		if (searchText.length !== 0) {
			setData(_.filter(operatorDetails, (item) => objToString(item).toLowerCase().includes(searchText.toLowerCase())));
			setPage(0);
		} else {
										...
										<TableRow
											...
											<TableCell component="th" scope="row" align="left">
												<Tooltip title="Activate / Deactivate" arrow>
													<Button onClick={(event) => handleDeleteRow(event, n.id)}>
														{n.is_active ? (
															<Icon className="text-green text-20" onClick={(e) => toggleChecked(e, n)}>
																check_circle
															</Icon>
														) : (
														...
														<Dialog aria-labelledby="confirmation-dialog-title" open={open} onClose={handleCancel}>
															<DialogTitle id="confirmation-dialog-title">Change Admin Operator Active Status</DialogTitle>
															<DialogContent>Do you want to change this Admin Operator's status? </DialogContent>
															<DialogActions>
											...
											<TableCell component="th" scope="row" align="left">
												<Button onClick={(event) => handleDeleteRow(event, n.id)}>
													...
													<Dialog aria-labelledby="confirmation-dialog-title" open={open1} onClose={handleCancel1}>
														<DialogTitle id="confirmation-dialog-title">Delete Operator</DialogTitle>
